...
class reconstructXYZ():
    def __init__(self):
        ...
        self.camera_x_offset = 0.25
        self.camera_y_offset = 0.0
        self.camera_z_offset = -0.1
    ...
    def transformMeasurements(self, t_x, t_y, t_z):
        ...
        uav_e = self.quaternion2euler(quaternion)
        euler = [0.0 , 0.0, uav_e[2]]
        position = [self.odometry_data.pose.pose.position.x + self.camera_x_offset, self.odometry_data.pose.pose.position.y + self.camera_y_offset, self.odometry_data.pose.pose.position.z + self.camera_z_offset]

        Tgazebo_uav = self.getRotationTranslationMatrix(euler, position)

        Tpos_world = np.dot(Tgazebo_uav, Tpos_uav)
