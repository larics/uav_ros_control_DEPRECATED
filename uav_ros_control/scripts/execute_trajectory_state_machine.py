...
from geometry_msgs.msg import Twist, Transform, Pose, PoseStamped
...
class UavExplorationSm:
  ...
  def __init__(self):
    ...
    self.executing_pub = rospy.Publisher('exploration/executing', Int32, 
      queue_size=1)
    self.point_reached_pub = rospy.Publisher('exploration_sm/point_reached', Bool, 
      queue_size=1)
    ...
    rospy.Subscriber('exploration_sm/goal', PoseStamped, 
  ...
  def checkTrajectoryExecuted(self):
    ...
    for i in range(self.n_feedback_points):
      ...
      dyaw = abs(self.quaternion2Yaw(self.target_pose.orientation) - 
        self.quaternion2Yaw(self.feedback_array[i].orientation))
