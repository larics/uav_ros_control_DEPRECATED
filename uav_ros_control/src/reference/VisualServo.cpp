...
void uav_reference::VisualServo::initializeParameters(ros::NodeHandle& nh)
{
  ...
  ROS_INFO("VS - deadzones x,y,z = [%.3f, %.3f, %.3f]", 
    _deadzone_x, _deadzone_y, _deadzone_z);
...
void VisualServo::updateSetpoint() {
  ...
  if (_targetCentroid.point.x != -1 
    ...
    if(!_z_frozen) move_up = _x_axis_PID.compute(0, _targetCentroid.point.z, 1 / _rate);
    if(!_y_frozen) move_left = _y_axis_PID.compute(0, _targetCentroid.point.y, 1 / _rate);
